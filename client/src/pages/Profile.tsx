...
function Permission({ icon, label, status }: { icon: React.ReactNode; label: string; status: string }) { return <div className="flex items-center gap-3 rounded-xl bg-white/75 p-3 dark:bg-[#203232]"><span className="text-[#0b645c] dark:text-[#7ed4c5]">{icon}</span><div className="min-w-0"><div className="text-xs font-bold text-[#245b55] dark:text-[#d8eee9]">{label}</div><div className="mt-0.5 flex items-center gap-1 text-[10px] text-[#5f8982] dark:text-[#a9c0bb]"><CheckCircle2 className="h-3 w-3" />{status}</div></div></div>; }
