      ...
      <aside className={`${mobileNavOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} fixed lg:sticky top-0 z-40 h-screen w-[252px] shrink-0 border-r border-slate-200 bg-white px-4 py-5 transition-transform duration-200`}>
        <div className="flex h-full flex-col">
          <div className="flex items-center gap-3 px-3">
            <div className="grid h-10 w-10 place-items-center rounded-[13px] bg-[#0b645c] text-sm font-extrabold text-white shadow-[0_8px_20px_rgba(11,100,92,.18)]">RP</div>
            <div>
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="px-4 py-6 sm:px-6 lg:px-9 lg:py-8">
          ...
          <section className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div><div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-[#0b645c]"><span className="h-2 w-2 rounded-full bg-[#f2bf56]" />Portfolio overview</div><h2 className="font-display text-[29px] font-bold tracking-tight">Keep rent moving forward.</h2><p className="mt-1.5 max-w-xl text-sm leading-6 text-slate-500">One calm view of your properties, tenant balances, and the conversations that keep everyone on track.</p></div>
          ...
          <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard label="Outstanding balance" value={`$${totalDebt}`} helper={`${overdueCount} tenants need attention`} icon={<CircleDollarSign className="h-5 w-5" />} tone="warm" />
            <StatCard label="Collected this cycle" value={`$${stats?.collectedThisMonth ?? "0.00"}`} helper="Payment receipts delivered" icon={<CheckCircle2 className="h-5 w-5" />} tone="mint" />
            <StatCard label="Active tenants" value={String(stats?.totalTenants ?? 0)} helper={`${stats?.totalProperties ?? 0} properties in portfolio`} icon={<Users className="h-5 w-5" />} tone="cream" />
          ...
          <section className="mt-6 grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
            <div className="relative overflow-hidden rounded-2xl bg-[#e5f1ee] p-5 sm:p-6"><div className="absolute -right-8 -top-10 h-44 w-44 rounded-full bg-[#cce5df] opacity-70" /><div className="relative"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-[#0b645c]"><ShieldCheck className="h-4 w-4" />The RentPulse promise</div><h3 className="mt-3 max-w-xl font-display text-xl font-bold tracking-tight text-[#153d39]">Every payment closes the loop.</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#416c67]">When a payment is recorded, the tenant gets their updated balance, the amount left to pay, and the date they need to act by — automatically.</p><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-lg bg-white/75 px-3 py-2 text-xs font-semibold text-[#245b55]">Balance recalculated</span><span className="rounded-lg bg-white/75 px-3 py-2 text-xs font-semibold text-[#245b55]">Receipt delivered</span><span className="rounded-lg bg-white/75 px-3 py-2 text-xs font-semibold text-[#245b55]">Ledger updated</span></div></div></div>
          ...
          <section className="mt-8 grid items-start gap-7 xl:grid-cols-[1.55fr_.75fr]">
            ...
            <div className="min-w-0">
              <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h3 className="font-display text-xl font-bold tracking-tight">Tenant portfolio</h3><p className="mt-1 text-xs text-slate-500">The people and balances that need your attention today.</p></div><div className="flex items-center gap-2"><div className="relative"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search tenants" className="h-9 w-40 border-slate-200 bg-white pl-8 text-xs sm:w-48" /></div><Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="h-9 w-[104px] border-slate-200 bg-white text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All status</SelectItem><SelectItem value="debt">With debt</SelectItem><SelectItem value="paid">Settled</SelectItem><SelectItem value="overdue">Overdue</SelectItem></SelectContent></Select></div></div>
              <div className="mt-4 overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200"><div className="flex items-center justify-between border-b border-slate-100 px-4 py-3.5 sm:px-5"><div className="flex items-center gap-2 text-xs font-bold text-slate-700"><Users className="h-4 w-4 text-[#0b645c]" />{filteredTenants.length} tenants shown</div><Select value={propertyFilter} onValueChange={setPropertyFilter}><SelectTrigger className="h-8 w-[150px] border-slate-200 text-[11px]"><Building2 className="mr-1 h-3.5 w-3.5 text-slate-400" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All properties</SelectItem>{properties.map(p => <SelectItem key={p.id} value={String(p.id)}>{p.name}</SelectItem>)}</SelectContent></Select></div>
                {tenantsLoading ? <div className="py-16 text-center text-sm text-slate-400">Loading portfolio…</div> : filteredTenants.length === 0 ? <div className="py-16 text-center text-sm text-slate-400">No tenants match those filters.</div> : <div className="divide-y divide-slate-100">{filteredTenants.map(t => { const balance = parseFloat(t.currentBalance); const hasDebt = balance > 0; return <div key={t.id} className="group flex flex-col justify-between gap-4 px-4 py-4 transition hover:bg-[#fbfcfc] sm:flex-row sm:items-center sm:px-5"><div className="flex min-w-0 items-center gap-3"><div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl text-xs font-bold ${hasDebt ? "bg-[#fff0e5] text-[#c36b38]" : "bg-[#e7f4ee] text-[#238059]"}`}>{t.fullName.split(" ").map((n: string) => n[0]).join("").slice(0,2)}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><button onClick={() => setDetailTenantId(t.id)} className="truncate text-left text-sm font-bold text-slate-900 hover:text-[#0b645c]">{t.fullName}</button><span className="rounded-md bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] font-bold text-slate-500">UNIT {t.unitNumber}</span></div><div className="mt-1 flex flex-wrap items-center gap-x-2 text-[11px] text-slate-400"><span>{t.propertyName}</span><span>·</span><span>Due {t.dueDayOfMonth}th</span><span>·</span><span>{t.phone}</span></div></div></div><div className="flex items-center justify-between gap-4 pl-[52px] sm:justify-end sm:pl-0"><div className="text-left sm:text-right"><div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Balance</div><div className={`font-display text-base font-bold ${hasDebt ? "text-[#d45753]" : "text-[#238059]"}`}>${t.currentBalance}</div></div><div className="flex items-center gap-1.5"><Button size="sm" variant="outline" onClick={() => setRemindTenant(t)} className="h-8 border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-600"><Bell className="mr-1 h-3.5 w-3.5 text-[#0b645c]" />Remind</Button><Button size="sm" onClick={() => setPaymentTenant(t)} className="h-8 bg-[#0b645c] px-2.5 text-xs font-bold hover:bg-[#09564f]"><CreditCard className="mr-1 h-3.5 w-3.5" />Pay</Button></div></div></div>; })}</div>}
              </div>
            </div>
