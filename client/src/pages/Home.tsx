      ...
      <aside className={`${mobileNavOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} fixed lg:sticky top-0 z-40 h-screen w-[252px] shrink-0 border-r border-slate-200 bg-white px-4 py-5 transition-transform duration-200`}>
        <div className="flex h-full flex-col">
          <div className="flex items-center gap-3 px-3">
            <div className="grid h-10 w-10 place-items-center rounded-[13px] bg-white p-1.5 shadow-[0_8px_20px_rgba(11,100,92,.18)] dark:bg-[#e8f7f2]"><img src="/icons/rentpulse.svg" alt="RentPulse logo" className="h-full w-full object-contain" /></div>
            <div>
          ...
          <nav className="mt-2 space-y-1">
            ...
            <button onClick={() => setIsHelpOpen(true)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[13px] font-semibold text-slate-500 transition hover:bg-slate-50 hover:text-slate-900"><LifeBuoy className="h-[17px] w-[17px]" />Help center</button>
          </nav>
        ...
        <div className="px-4 py-6 sm:px-6 lg:px-9 lg:py-8">
          ...
          <section className="mt-6 grid gap-4 lg:grid-cols-[1.15fr_.85fr_1fr]">
            <div className="rounded-2xl bg-white p-5 ring-1 ring-slate-200">
              ...
              <div className="mt-3 flex items-end justify-between gap-3"><div className="font-display text-3xl font-bold">{collectionRate}%</div><div className="pb-1 text-right text-[11px] text-slate-500">KSh {collectedThisCycle.toFixed(2)} collected<br />of KSh {portfolioDue.toFixed(2)} due</div></div>
            ...
            <div className="rounded-2xl bg-white p-5 ring-1 ring-slate-200"><div className="flex items-center justify-between"><div className="text-[11px] font-bold uppercase tracking-[.13em] text-slate-400">Average debt / tenant</div><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#fff4ec] text-[#d45753]"><Wallet className="h-4 w-4" /></div></div><div className="mt-3 font-display text-3xl font-bold">KSh {averageDebt.toFixed(2)}</div><p className="mt-1 text-xs text-slate-500">Across {tenants.length} tracked tenants</p></div>
